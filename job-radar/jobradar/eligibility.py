"""Deterministic location eligibility checks for JobRadar jobs."""
...
INDIA_PATTERNS = (r"\bindia\b", r"\bindian\b", r"\bist\b", r"india standard time")
GLOBAL_PATTERNS = (r"\bworldwide\b", r"\bglobal(?:ly)?\b", r"\banywhere\b", r"\bwork from anywhere\b", r"\bremote\s*[- ]?first\b")
RESTRICTED_COUNTRY_PATTERNS = (r"\b(?:united states|u\.?s\.?a?|us)\b", r"\bcanada\b", r"\b(?:united kingdom|u\.?k\.?)\b", r"\b(?:european union|eu)\b", r"\baustralia\b", r"\bnew zealand\b", r"\bgermany\b", r"\bfrance\b")
...
def classify_india_eligibility(*, location: str = "", location_restrictions: object = "", country: str = "", timezone: str = "", description: str = "", remote: bool = False) -> str:
    ...
    if _has(all_text, INDIA_PATTERNS) or _has(all_text, GLOBAL_PATTERNS):
    ...
    # Descriptions can mention offices, so require eligibility language there.
