...
SOURCE_TYPES = {"API", "RSS", "Career Portal", "Company Career Page", "Telegram", "Custom"}
...
class LegacySourceAdapter(JobSource):
    ...
    def fetch(self, query: str, *, location: str = "", limit: int = 50, max_pages: int = 3, companies_path: str = "companies.yaml") -> List[Job]:
        kwargs: Dict[str, Any] = {"limit": limit, "max_pages": max_pages}
...
_BUILT_INS = {
    "remotive": ("Remotive", "API", RemotiveSearch, {}),
    "arbeitnow": ("Arbeitnow", "API", ArbeitnowSearch, {}),
    "remoteok": ("RemoteOK", "API", RemoteOKSearch, {}),
    "jobicy": ("Jobicy", "API", JobicySearch, {}),
    "himalayas": ("Himalayas", "API", HimalayasSearch, {}),
    "greenhouse": ("Greenhouse", "Career Portal", GreenhouseSearch, {"provider": "Greenhouse"}),
    "ashby": ("Ashby", "Career Portal", AshbySearch, {"provider": "Ashby"}),
    "linkedin": ("LinkedIn", "Custom", LinkedInSearch, {"warning": "Opt-in public HTML source"}),
}
...
class SourceRegistry:
    ...
    def _register_built_ins(self) -> None:
        for source_id, (name, source_type, source_class, default_metadata) in _BUILT_INS.items():
            config = self._config.get(source_id, {})
            if not isinstance(config, dict):
                config = {}
            metadata = {**default_metadata, **(config.get("metadata", {}) or {})}
            self.register(LegacySourceAdapter(
                source_id=source_id,
                name=config.get("name", name),
                source_type=config.get("type", source_type),
                enabled=bool(config.get("enabled", source_id != "linkedin")),
                metadata=metadata,
                search=source_class.search,
                error_getter=lambda cls=source_class: getattr(cls, "last_error", ""),
                supports_location=source_id == "linkedin",
                needs_companies=source_id in {"greenhouse", "ashby"},
            ))
    ...
    def enabled_sources(self, overrides: Dict[str, bool] | None = None) -> List[JobSource]:
        overrides = overrides or {}
        return [source for source in self._sources.values() if overrides.get(source.source_id, source.enabled)]
